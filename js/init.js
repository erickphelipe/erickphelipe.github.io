...
(function ($) {
  ...
  var FrenifyOzzy = {
    init: function () {
      FrenifyOzzy.animatedText();
      FrenifyOzzy.imgToSVG();
      FrenifyOzzy.BgImg();

      FrenifyOzzy.headerTrigger();
      FrenifyOzzy.menuFixer();
      FrenifyOzzy.timeLine();
      FrenifyOzzy.movingBlog();
      FrenifyOzzy.navFixer();
      FrenifyOzzy.anchor();
      FrenifyOzzy.movingTextForNav();
      FrenifyOzzy.filterItems();
      FrenifyOzzy.applyFilter();
      FrenifyOzzy.magnific();
      FrenifyOzzy.progressTotop();
      FrenifyOzzy.productModal();
      FrenifyOzzy.contactForm();
      FrenifyOzzy.moreCategories();
      FrenifyOzzy.totop();
      FrenifyOzzy.vegasSlider();
      FrenifyOzzy.heroJarallaxSlider();
      FrenifyOzzy.subscribe();
    },
    ...
    subscribe: function () {
      $(".subscribe_form a").on("click", function () {
        ...
        if (email === "") {
          ...
          if (FrenifyOzzy.validateEmail(email)) {
            $.post(
              "modal/subscribe.php",
    ...
    vegasSlider: function () {
      $(".ozzy_fn_hero .vegas-slide").each(function () {
        var element = $(this);
        var html = [];
    ...
    heroJarallaxSlider: function () {
      // slider
      var section = $(".ozzy_fn_hero .swiper-container");
    ...
    moreCategories: function () {
      $(".ozzy_fn_categories").each(function () {
        var element = $(this);
      ...
      FrenifyOzzy.showMore();
    },

    showMore: function () {
      $(".ozzy_fn_categories .more")
        .off()
        .on("click", function () {
          var element = $(this),
            parent = element.closest(".ozzy_fn_categories"),
    ...
    productModal: function () {
      var modal = $(".ozzy_fn_modal.product_modal");
      $(".ozzy_fn_drops .item a")
        .off()
        .on("click", function () {
          var element = $(this);
          ...
          modal.find(".ozzy_fn_title .fn_title").text(title);
    ...
    applyFilter: function () {
      ...
      FrenifyOzzy.isotopeCollection();

      // left filter on click function
      $(".ozzy_fn_filters .checkbox")
        .off()
        .on("click", function () {
          // our clicked filter
          var element = $(this),
            // collection wrapper
            parent = element.closest(".ozzy_fn_collection"),
            // filter result box
            resultBox = parent.find(".ozzy_fn_result_box"),
          ...
          // if clicked item has already clicked and clicked second time
            ...
            var index = OzzyFilterArray.indexOf(id);
            if (index !== -1) {
              OzzyFilterArray.splice(index, 1);
            }
          }

          FrenifyOzzy.recallGridAfterFiltering();
    ...
    recallGridAfterFiltering: function (clear) {
      ...
      if (OzzyFilterArray.length === 0) {
        filters = "*";
      }
      // if there has one or more selected filters
      else {
        $.each(OzzyFilterArray, function (index, value) {
          filters += ".id" + value;
        });
      }
      console.log(OzzyFilterArray);
      // run isotope after filter has been clicked
      $grid.isotope({ filter: filters });
    },

    removeFilter: function () {
      $(".ozzy_fn_result_box .result_item a")
        .off()
        .on("click", function () {
          var e = $(this),
            parent = e.closest(".ozzy_fn_collection"),
          ...
          resultItem.remove();
          parent
            .find('.ozzy_fn_filters .checkbox[data-id="' + id + '"]')
      ...
      $(".ozzy_fn_result_box .clear_all")
        .off()
        .on("click", function () {
          var e = $(this),
            parent = e.closest(".ozzy_fn_collection"),
            filterBox = parent.find(".ozzy_fn_filters"),
            resultBox = parent.find(".ozzy_fn_result_box"),
            filterCount = resultBox.find(".filter_count span");
          filterCount.text(0);
          ...
          filterBox
            ...
            .find('input[type="checkbox"]')
            .prop("checked", "");
          FrenifyOzzy.recallGridAfterFiltering("clear");
          return false;
        });
    },
    ...
    movingTextForNav: function () {
      ...
      if (overlay.length) {
        if (!$(".ozzy_fn_moving_text").length) {
          $("body").append('<div class="neoh_fn_moving_text">Close</div>');
        }
        var movingBox = $(".ozzy_fn_moving_text");
        overlay
    ...
    anchor: function () {
      $(".ozzy_fn_down").on("click", function () {
        var e = $(this),
          href = e.attr("href");
        if (
    ...
    navFixer: function () {
      var footerHeight = $(".ozzy_fn_nav .nav_footer").outerHeight();
      $(".ozzy_fn_nav .nav_content").css({
        height: $(window).height() - footerHeight + "px",
      });
    },

    movingBlog: function () {
      var blog = $(".ozzy_fn_moving_blog");
      if (blog.length) {
        if (!$(".ozzy_fn_moving_box").length) {
          $("body").append('<div class="neoh_fn_moving_box">');
        }
        var movingBox = $(".ozzy_fn_moving_box");
        var list = $(".ozzy_fn_moving_blog .item");
        list
          .on("mouseenter", function (event) {
            var element = $(this);
    ...
    timeLine: function () {
      var allContentItems = $(".ozzy_fn_timeline .timeline_item");
      var progressLine = $(".ozzy_fn_timeline .progress_line");
      var allProgressItems = $(".ozzy_fn_timeline .progress_line li");
      var activeProgressItem = $(".ozzy_fn_timeline .progress_line .active");
      var activeLine = $(".ozzy_fn_timeline .progress_line .active_line");
      var allProgressButtons = $(".ozzy_fn_timeline .progress_line a");
      var initialSpace = 110;
      var spaceBetweenItems = 230;

      FrenifyOzzy.timelineClasses(allContentItems, allProgressItems, "initial");
      ...
      allProgressButtons.off().on("click", function () {
        ...
        if (!p.hasClass("active")) {
          var timeline = e.closest(".ozzy_fn_timeline");
          var activeIndex = p.data("index");
          FrenifyOzzy.timelineClasses(
            allContentItems,
            allProgressItems,
            activeIndex
          );
      ...
      var nextActive = true;
      var prevActive = true;
      var prevButton = $(".ozzy_fn_timeline .nav_prev");
      var nextButton = $(".ozzy_fn_timeline .nav_next");
    ...
    timelineClasses: function (allContentItems, allProgressItems, activeIndex) {
      ...
      isActive = false;
      if (activeIndex === "initial") {
        extraActiveIndex = $(".ozzy_fn_timeline .progress_line .active").data(
          "index"
        );
      } else {
    ...
    headerTrigger: function () {
      var trigger = $(".ozzy_fn_header .trigger");
      var navOverlay = $(".nav_overlay");
      var rightNav = $(".ozzy_fn_nav");
      var trigger2 = rightNav.find(".trigger");
      var links = $(".ozzy_fn_nav .nav_menu a");
      var buttons = $(".ozzy_fn_nav .nav_buttons");
      var footer = $(".ozzy_fn_nav .nav_footer");

      var menuItems = $(".ozzy_fn_nav .nav_menu > ul > li");
